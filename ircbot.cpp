	...
	if (msg.find("!p") == 0) {
		...
		if (file.is_open()) {
			...
			if (msg.empty()) {
				...
				if (query.err.empty()) {
					sendMsg("players: " + std::to_string((int)query.response.cur) + "/" + std::to_string((int)query.response.max) + " | " + query.response.name + " | " + query.response.map + " | steam://connect/" + servers[0].server + ":" + std::to_string(query.response.edf_data.port), chan);
				} else
				...
				for (int i = 0; i < servers.size(); i++) {
					if (servers[i].prefix == msg) {
						...
						if (query.err.empty()) {
							sendMsg("players: " + std::to_string((int)query.response.cur) + "/" + std::to_string((int)query.response.max) + " | " + query.response.name + " | " + query.response.map + " | steam://connect/" + servers[i].server + ":" + std::to_string(query.response.edf_data.port), chan);
						} else
							sendMsg(query.err, chan);
					}
				}
